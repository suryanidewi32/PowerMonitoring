...
export default class ChartConsumption extends Component {
    ...
    super(props);
      ...
      // const age=[];
      const time=[];
        await axios.get('http://localhost:3002/kwhselisih')
        .then(Response=>{
        console.log("Response", Response.data.kwh);
        for(const obj of Response.data.kwh)
        {
            console.log("obj", obj);
            selisih.push(obj.selisih);
            // age.push(obj.employee_age);
            time.push(obj.time);
        }
        console.log("salary", selisih);
        this.setState({
            options: {
                chart: {
                  id: 'apexchart-example'
...
// class ChartConsumption extends React.Component {
//   render() {
//     return (
//         <ReactFC
//         type="msarea"
